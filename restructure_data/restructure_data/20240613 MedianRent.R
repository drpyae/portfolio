...
#install.packages("tidycensus","reshape2","janitor","googlesheets4")
...
df_new$value <- as.numeric(df_new$value)
df_new2 <- df_new %>%
  filter(!(ZipCode %in% c("mean","median")))

#==================================================
# Calculate mean and median for 'value' for each year
#================================================
summary_df <- df_new2 %>%
  group_by(year) %>%
  summarize(
...
# Convert summary_df to long format to match the original dataframe structure
mean_rows <- summary_df %>%
  mutate(type = "mean") %>%
  select(year, type, mean_value) %>%
  rename(value = mean_value)

median_rows <- summary_df %>%
  mutate(type = "median") %>%
  select(year, type, median_value) %>%
  rename(value = median_value)

# Combine mean and median rows
summary_long <- bind_rows(mean_rows, median_rows)

# Add a column to indicate the type of row (original, mean, median)
df_new2$type <- "original"

# Combine the original dataframe with the summary_long dataframe
year_df <- bind_rows(df_new2, summary_long)

# Print the final dataframe
#print(year_df)

#==========================================
#Calculate Mean and Median for each Category . You may ignore mean and median calculation for each year from above codes.
#=========================================
...
# Convert summary_df to long format to match the original data frame structure
mean_rows <- summary_df2 %>%
  mutate(type = "mean") %>%
  select(Category, type, mean_value) %>%
  rename(value = mean_value)

median_rows <- summary_df2 %>%
  mutate(type = "median") %>%
  select(Category, type, median_value) %>%
  rename(value = median_value)

# Combine mean and median rows
summary_long2 <- bind_rows(mean_rows, median_rows)

# Add a column to indicate the type of row (original, mean, median)
#df_new2$type <- "original"

# Combine the original dataframe with the summary_long data frame
final_df <- bind_rows(year_df, summary_long2)
...
sheet_write(final_df, ss = MedianRent, sheet = "MedianRent")
